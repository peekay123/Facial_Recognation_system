...
def Login(request):
    if request.method == "POST":
        ...
        if user_obj is None:
            messages.error(request, "User not found, plaese try with valid username !!!")
            return redirect('login')
        
        profile_obj = Profile.objects.filter(user=user_obj).first()
        if profile_obj is not None:
            if not profile_obj.is_verified:
                messages.error(request, "Your account is not verified. Please check your email to verify your account.")
                return redirect('login')
        else:
