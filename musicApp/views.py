...
def MusicPlay(request):
    ...
    music_list = list(Music.objects.filter(category=category))  # Replace 'Music' with your actual model name
    random.shuffle(music_list)  # Shuffle the music list
    if len(music_list) < 6:
        random_music = music_list  # Fetch all music objects
    else:
        random_music = music_list[:6]  # Fetch only 6 music objects
    image_data = context['image_data']
    context = {'music_list': random_music,'category':category,'image_data':image_data}
...
def FaceDetection(request):
    ...
    if request.method == 'POST':
        ...
        if img.shape[0] < 48 or img.shape[1] < 48:
            error_message = 'Image too small to detect your face. Please upload a bigger image between 48x48px to 2000x2000px.'
            return render(request, 'Expression-Detection.html', {'error_message': error_message})
